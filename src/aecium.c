...
static void uniquenessCheck(struct infoset * pinfo)
{
	struct hash *phs = pinfo -> phs;
	int otherPid = getOtherPid(phs -> programName);

	if ( otherPid ) {
		if ( phs -> leave ) {
			if ( kill(otherPid, SIGKILL) == -1 ) {
				perror("kill");
				exit(EXIT_FAILURE);
			}
		} else {
			fputs("You have accessed Internet yet!\n", stderr);
			exit(EXIT_SUCCESS);
		}
	} else {
		if ( phs -> leave ) {
			fputs("The procedure had not run yet!\n", stderr);
			exit(EXIT_SUCCESS);
		}
	}
}
...
{
	char *envLocale = setlocale(LC_CTYPE, "");
	if ( envLocale ) {
		char *encoding = strchr(envLocale, '.');
		...
		if ( encoding ) {
			...
			if ( (iconv_t)-1 != (cd = iconv_open(encoding, "GB18030")) ) {
